"""H-CRL learner implementation."""
import time
from typing import Any, Dict, Iterator, List, NamedTuple, Optional, Tuple, Callable

import acme
from acme.core import Learner
from acme import types
from acme.jax import networks as networks_lib
from acme.jax import utils
from acme.utils import counting
from acme.utils import loggers
from contrastive import config 
from contrastive import hnetworks 
import jax
import jax.numpy as jnp
import optax
import reverb
from jax.experimental.host_callback import id_print
from jax import debug
from jax.scipy.special import logsumexp
import numpy as np
from jax import random
import os
from default import make_default_logger
from pathlib import Path



class TrainingState(NamedTuple):
    policy_optimizer_state: optax.OptState
    q_optimizer_state: optax.OptState
    policy_params: dict
    policy_params_prev: dict
    q_params: dict
    target_q_params: dict
    key: jax.random.KeyArray
    alpha_optimizer_state: Optional[optax.OptState] = None
    alpha_params: Optional[jnp.ndarray] = None


class ContrastiveLearner(Learner):
    """Contrastive RL Learner implementation."""

    def __init__(
        self,
        hnetworks,
        rng,
        policy_optimizer,
        q_optimizer,
        iterator,
        counter,
        logger,
        obs_to_goal,
        config,
    ):
        self._hnetworks = hnetworks
        self._iterator = iterator
        self._counter = counter or counting.Counter()
        self._logger = logger or make_default_logger('learner', asynchronous=True, serialize_fn=utils.fetch_devicearray, time_delta=10.0)
        self._obs_to_goal = obs_to_goal
        self._config = config
        self._num_sgd_steps_per_step = config.num_sgd_steps_per_step
        self._use_td = config.use_td
        self._obs_dim = config.obs_dim
        self._adaptive_entropy = config.entropy_coefficient is None

        if config.add_mc_to_td:
            assert config.use_td

        if self._adaptive_entropy:
            self._log_alpha = jnp.asarray(0., dtype=jnp.float32)
            self._alpha_optimizer = optax.adam(learning_rate=3e-4)
            self._alpha_optimizer_state = self._alpha_optimizer.init(self._log_alpha)
        elif config.target_entropy:
            raise ValueError("Provide either entropy_coefficient or target_entropy, not both.")

        # Compile update step
        self._update_step = self._create_update_step()
        if config.jit:
            self._update_step = jax.jit(self._update_step)

        self._state = self._make_initial_state(rng)
        self._timestamp = None

    def _make_initial_state(self, key):
        key_policy, key_q, key = random.split(key, 3)
        policy_params = self._hnetworks.policy_network.init(key_policy)
        q_params = self._hnetworks.q_network.init(key_q)
        state = TrainingState(
            policy_optimizer_state=self._config.policy_optimizer.init(policy_params),
            q_optimizer_state=self._config.q_optimizer.init(q_params),
            policy_params=policy_params,
            policy_params_prev=policy_params,
            q_params=q_params,
            target_q_params=q_params,
            key=key
        )
        if self._adaptive_entropy:
            state = state._replace(
                alpha_optimizer_state=self._alpha_optimizer_state,
                alpha_params=self._log_alpha,
            )
        return state

    def _create_update_step(self):
        """Returns the full update function with gradients and metrics."""

        def update(state, transitions):
            key, key_alpha, key_critic, key_actor = jax.random.split(state.key, 4)

            # Alpha (entropy) loss
            if self._adaptive_entropy:
                alpha_loss, alpha_grads = jax.value_and_grad(self._alpha_loss)(
                    state.alpha_params, state.policy_params, transitions, key_alpha
                )
                alpha = jnp.exp(state.alpha_params)
            else:
                alpha = self._config.entropy_coefficient

            # Critic loss
            (critic_loss, critic_metrics), critic_grads = jax.value_and_grad(self._critic_loss, has_aux=True)(
                state.q_params, state.policy_params, state.target_q_params, transitions, key_critic
            )
            q_update, q_opt_state = self._config.q_optimizer.update(critic_grads, state.q_optimizer_state)
            q_params = optax.apply_updates(state.q_params, q_update)

            # Soft update of target network
            target_q_params = jax.tree_map(
                lambda x, y: x * (1 - self._config.tau) + y * self._config.tau,
                state.target_q_params, q_params
            )

            # Actor loss
            (actor_loss, actor_metrics), actor_grads = jax.value_and_grad(self._actor_loss, has_aux=True)(
                state.policy_params, state.q_params, alpha, transitions, key_actor
            )
            actor_update, policy_opt_state = self._config.policy_optimizer.update(actor_grads, state.policy_optimizer_state)
            policy_params = optax.apply_updates(state.policy_params, actor_update)

            new_state = TrainingState(
                policy_optimizer_state=policy_opt_state,
                q_optimizer_state=q_opt_state,
                policy_params=policy_params,
                policy_params_prev=state.policy_params,
                q_params=q_params,
                target_q_params=target_q_params,
                key=key,
            )

            metrics = {
                'critic_loss': critic_loss,
                'actor_loss': actor_loss,
                **critic_metrics,
                **actor_metrics,
            }

            if self._adaptive_entropy:
                alpha_update, alpha_opt_state = self._alpha_optimizer.update(
                    alpha_grads, state.alpha_optimizer_state)
                alpha_params = optax.apply_updates(state.alpha_params, alpha_update)
                new_state = new_state._replace(
                    alpha_optimizer_state=alpha_opt_state,
                    alpha_params=alpha_params)
                metrics.update({
                    'alpha_loss': alpha_loss,
                    'alpha': jnp.exp(alpha_params),
                })

            return new_state, metrics

        return utils.process_multiple_batches(update, self._config.num_sgd_steps_per_step)

    def _alpha_loss(self, log_alpha, policy_params, transitions, key):
        dist_params = self._hnetworks.policy_network.apply(policy_params, transitions.observation)
        actions = self._hnetworks.sample(dist_params, key)
        log_probs = self._hnetworks.log_prob(dist_params, actions)
        alpha = jnp.exp(log_alpha)
        loss = alpha * jax.lax.stop_gradient(-log_probs - self._config.target_entropy)
        return jnp.mean(loss)

    def critic_loss(q_params,
                    policy_params,
                    target_q_params,
                    transitions,
                    key):
      batch_size = transitions.observation.shape[0]
      # Note: We might be able to speed up the computation for some of the
      # baselines to making a single network that returns all the values. This
      # avoids computing some of the underlying representations multiple times.
      if config.use_td:
        # For TD learning, the diagonal elements are the immediate next state.
        s, g = jnp.split(transitions.observation, [config.obs_dim], axis=1)
        next_s, _ = jnp.split(transitions.next_observation, [config.obs_dim],
                              axis=1)
        if config.add_mc_to_td:
          next_fraction = (1 - config.discount) / ((1 - config.discount) + 1)
          num_next = int(batch_size * next_fraction)
          new_g = jnp.concatenate([
              obs_to_goal(next_s[:num_next]),
              g[num_next:],
          ], axis=0)
        else:
          new_g = obs_to_goal(next_s)
        obs = jnp.concatenate([s, new_g], axis=1)
        transitions = transitions._replace(observation=obs)
      I = jnp.eye(batch_size)  # pylint: disable=invalid-name
      
      logits, _, _ = hnetworks.q_network.apply(q_params, transitions.observation, transitions.action)

      if config.use_td:
        # Make sure to use the twin Q trick.
        assert len(logits.shape) == 3

        # We evaluate the next-state Q function using random goals
        s, g = jnp.split(transitions.observation, [config.obs_dim], axis=1)
        del s
        next_s = transitions.next_observation[:, :config.obs_dim]
        goal_indices = jnp.roll(jnp.arange(batch_size, dtype=jnp.int32), -1)
        g = g[goal_indices]
        transitions = transitions._replace(
            next_observation=jnp.concatenate([next_s, g], axis=1))
        next_dist_params = hnetworks.policy_network.apply(
            policy_params, transitions.next_observation)
        next_action = hnetworks.sample(next_dist_params, key)
        
        next_q, _, _ = hnetworks.q_network.apply(target_q_params,
                                          transitions.next_observation,
                                          next_action)  # This outputs logits.
        next_q = jax.nn.sigmoid(next_q)
        next_v = jnp.min(next_q, axis=-1)
        next_v = jax.lax.stop_gradient(next_v)
        next_v = jnp.diag(next_v)
        # diag(logits) are predictions for future states.
        # diag(next_q) are predictions for random states, which correspond to
        # the predictions logits[range(B), goal_indices].
        # So, the only thing that's meaningful for next_q is the diagonal. Off
        # diagonal entries are meaningless and shouldn't be used.
        w = next_v / (1 - next_v)
        w_clipping = 20.0
        w = jnp.clip(w, 0, w_clipping)
        # (B, B, 2) --> (B, 2), computes diagonal of each twin Q.
        pos_logits = jax.vmap(jnp.diag, -1, -1)(logits)
        loss_pos = optax.sigmoid_binary_cross_entropy(
            logits=pos_logits, labels=1)  # [B, 2]

        neg_logits = logits[jnp.arange(batch_size), goal_indices]
        loss_neg1 = w[:, None] * optax.sigmoid_binary_cross_entropy(
            logits=neg_logits, labels=1)  # [B, 2]
        loss_neg2 = optax.sigmoid_binary_cross_entropy(
            logits=neg_logits, labels=0)  # [B, 2]

        if config.add_mc_to_td:
          loss = ((1 + (1 - config.discount)) * loss_pos
                  + config.discount * loss_neg1 + 2 * loss_neg2)
        else:
          loss = ((1 - config.discount) * loss_pos
                  + config.discount * loss_neg1 + loss_neg2)
        # Take the mean here so that we can compute the accuracy.
        logits = jnp.mean(logits, axis=-1)

      else:  # For the MC losses.
        def loss_fn(_logits):  # pylint: disable=invalid-name
          if config.use_cpc:
            return (optax.softmax_cross_entropy(logits=_logits, labels=I)
                    + 0.01 * jax.nn.logsumexp(_logits, axis=1)**2)
          else:
            return optax.sigmoid_binary_cross_entropy(logits=_logits, labels=I)
        if len(logits.shape) == 3:  # twin q
          # loss.shape = [.., num_q]
          loss = jax.vmap(loss_fn, in_axes=2, out_axes=-1)(logits)
          loss = jnp.mean(loss, axis=-1)
          # Take the mean here so that we can compute the accuracy.
          logits = jnp.mean(logits, axis=-1)
        else:
          loss = loss_fn(logits)

      loss = jnp.mean(loss)
      correct = (jnp.argmax(logits, axis=1) == jnp.argmax(I, axis=1))
      logits_pos = jnp.sum(logits * I) / jnp.sum(I)
      logits_neg = jnp.sum(logits * (1 - I)) / jnp.sum(1 - I)
      if len(logits.shape) == 3:
        logsumexp = jax.nn.logsumexp(logits[:, :, 0], axis=1)**2
      else:
        logsumexp = jax.nn.logsumexp(logits, axis=1)**2
      metrics = {
          'binary_accuracy': jnp.mean((logits > 0) == I),
          'categorical_accuracy': jnp.mean(correct),
          'logits_pos': logits_pos,
          'logits_neg': logits_neg,
          'logsumexp': logsumexp.mean(),
      }

      return loss, metrics

    def actor_loss(policy_params,
                   q_params,
                   alpha,
                   transitions,
                   key,
                   ):
      obs = transitions.observation

      state = obs[:, :config.obs_dim]
      goal = obs[:, config.obs_dim:]

      if config.random_goals == 0.0:
        new_state = state
        new_goal = goal
      elif config.random_goals == 0.5:
        new_state = jnp.concatenate([state, state], axis=0)
        new_goal = jnp.concatenate([goal, jnp.roll(goal, 1, axis=0)], axis=0)
      else:
        assert config.random_goals == 1.0
        new_state = state
        new_goal = jnp.roll(goal, 1, axis=0)

      new_obs = jnp.concatenate([new_state, new_goal], axis=1) 
      dist_params = hnetworks.policy_network.apply(policy_params, new_obs)
      action = hnetworks.sample(dist_params, key)
      log_prob = hnetworks.log_prob(dist_params, action)

      q_action, sa_repr, sf_repr = hnetworks.q_network.apply(q_params, new_obs, action)

      if len(q_action.shape) == 3:  # twin q trick
        assert q_action.shape[2] == 2
        q_action = jnp.min(q_action, axis=-1)

      actor_loss = -jnp.diag(q_action) # negative -(Q): maximize Q

      # action entropy loss
      approx_entropy = -log_prob

      if config.use_action_entropy:
        actor_loss -= alpha * approx_entropy # negative -(-log prob): maximize entropy

      metrics = {
          'entropy_mean': jnp.mean(approx_entropy),
      }

      
      return jnp.mean(actor_loss), metrics

    def step(self):
        sample = next(self._iterator)
        transitions = types.Transition(*sample.data)
        self._state, metrics = self._update_step(self._state, transitions)

        timestamp = time.time()
        elapsed_time = timestamp - self._timestamp if self._timestamp else 0
        self._timestamp = timestamp

        counts = self._counter.increment(steps=1, walltime=elapsed_time)
        metrics['steps_per_second'] = (
            self._num_sgd_steps_per_step / elapsed_time if elapsed_time > 0 else 0.0)
        self._logger.write({**metrics, **counts})

    def get_variables(self, names):
        variables = {
            'policy': self._state.policy_params,
            'critic': self._state.q_params,
        }
        return [variables[name] for name in names]

    def save(self):
        return self._state

    def restore(self, state):
        self._state = state
